    ...
    def _perform_transaction(self, params: Dict[str, Any], req_id: Any) -> JsonResponse:
        ...
        with transaction.atomic():
            ...
            models.OrderStatusHistory.objects.create(
                ...
                comment=f"Payme orqali to'lov muvaffaqiyatli amalga oshirildi. ID: {payme_trans_id}"
            )

            models.AuditLog.objects.create(
                user=order.user,
                action="PAYMENT_SUCCESS_PAYME",
                details=f"Buyurtma #{order.code[:8]} uchun Payme to'lovi: {locked.amount} UZS. Tranzaksiya #{payme_trans_id}"
            )

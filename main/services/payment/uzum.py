        ...
        if event in ('SUCCESS', 'PAID', 'CONFIRMED'):
            with transaction.atomic():
                ...
                models.OrderStatusHistory.objects.create(
                    ...
                    comment=f"Uzum Bank orqali to'lov muvaffaqiyatli qabul qilindi. ID: {trans_id}"
                )

                models.AuditLog.objects.create(
                    user=order.user,
                    action="PAYMENT_SUCCESS_UZUM",
                    details=f"Buyurtma #{order.code[:8]} uchun Uzum Bank to'lovi: {locked.amount} UZS. Tranzaksiya #{trans_id}"
                )
